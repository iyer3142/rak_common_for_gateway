...
#define DEFAULT_KEEPALIVE \
  5 /* default time interval for downstream keep-alive packet */
#define DEFAULT_STAT 30 /* default time interval for statistics */
#define PUSH_TIMEOUT_MS 100
#define PULL_TIMEOUT_MS 200
#define GPS_REF_MAX_AGE                                                  \
  30 /* maximum admitted delay in seconds of GPS loss before considering \
        latest GPS sync unusable */
#define FETCH_SLEEP_MS 10 /* nb of ms waited when a fetch return no packets */
#define BEACON_POLL_MS 50 /* time in ms between polling of beacon TX status */

#define PROTOCOL_VERSION 2 /* v1.3 */

#define XERR_INIT_AVG                                                     \
  128 /* nb of measurements the XTAL correction is averaged on as initial \
         value */
...
#define MIN_LORA_PREAMB \
  6 /* minimum Lora preamble length for this application */
#define STD_LORA_PREAMB 8
#define MIN_FSK_PREAMB                                  \
  3 /* minimum FSK preamble length for this application \
     */
...
#define UNIX_GPS_EPOCH_OFFSET                                          \
  315964800 /* Number of seconds ellapsed between 01.Jan.1970 00:00:00 \
...
volatile bool exit_sig = false; /* 1 -> application terminates cleanly (shut
                                   down hardware, close open files, etc) */
volatile bool quit_sig =
    false; /* 1 -> application terminates without shutting down the hardware */

/* packets filtering configuration variables */
static bool fwd_valid_pkt =
    true; /* packets with PAYLOAD CRC OK are forwarded */
static bool fwd_error_pkt =
    false; /* packets with PAYLOAD CRC ERROR are NOT forwarded */
static bool fwd_nocrc_pkt =
    false; /* packets with NO PAYLOAD CRC are NOT forwarded */

/* network configuration variables */
static uint64_t lgwm = 0; /* Lora gateway MAC address */
static char serv_addr[64] =
    STR(DEFAULT_SERVER); /* address of the server (host name or IPv4/IPv6) */
static char serv_port_up[8] =
    STR(DEFAULT_PORT_UP); /* server port for upstream traffic */
static char serv_port_down[8] =
    STR(DEFAULT_PORT_DW); /* server port for downstream traffic */
static int keepalive_time =
    DEFAULT_KEEPALIVE; /* send a PULL_DATA request every X seconds, negative =
                          disabled */

/* statistics collection configuration variables */
static unsigned stat_interval =
    DEFAULT_STAT; /* time interval (in sec) at which statistics are collected
                     and displayed */
...
static struct timeval pull_timeout = {
    0, (PULL_TIMEOUT_MS * 1000)}; /* non critical for throughput */

/* hardware access control and correction */
pthread_mutex_t mx_concent =
    PTHREAD_MUTEX_INITIALIZER; /* control access to the concentrator */
static pthread_mutex_t mx_xcorr =
    PTHREAD_MUTEX_INITIALIZER; /* control access to the XTAL correction */
static bool xtal_correct_ok =
    false; /* set true when XTAL correction is stable enough */
static double xtal_correct = 1.0;

/* GPS configuration and synchronization */
static char gps_tty_path[64] =
    "\0";                        /* path of the TTY port GPS is connected on */
static int gps_tty_fd = -1;      /* file descriptor of the GPS TTY port */
static bool gps_enabled = false; /* is GPS enabled on that gateway ? */

/* GPS time reference */
static pthread_mutex_t mx_timeref =
    PTHREAD_MUTEX_INITIALIZER; /* control access to GPS time reference */
static bool gps_ref_valid; /* is GPS reference acceptable (ie. not too old) */
static struct tref time_reference_gps; /* time reference used for GPS <->
...
static pthread_mutex_t mx_meas_up =
    PTHREAD_MUTEX_INITIALIZER; /* control access to the upstream measurements */
static uint32_t meas_nb_rx_rcv = 0; /* count packets received */
static uint32_t meas_nb_rx_ok =
    0; /* count packets received with PAYLOAD CRC OK */
static uint32_t meas_nb_rx_bad =
    0; /* count packets received with PAYLOAD CRC ERROR */
static uint32_t meas_nb_rx_nocrc =
    0; /* count packets received with NO PAYLOAD CRC */
static uint32_t meas_up_pkt_fwd =
    0; /* number of radio packet forwarded to the server */
static uint32_t meas_up_network_byte =
    0; /* sum of UDP bytes sent for upstream traffic */
static uint32_t meas_up_payload_byte =
    0; /* sum of radio payload bytes sent for upstream traffic */
static uint32_t meas_up_dgram_sent =
    0; /* number of datagrams sent for upstream traffic */
static uint32_t meas_up_ack_rcv =
    0; /* number of datagrams acknowledged for upstream traffic */

static pthread_mutex_t mx_meas_dw =
    PTHREAD_MUTEX_INITIALIZER; /* control access to the downstream measurements
                                */
static uint32_t meas_dw_pull_sent =
    0; /* number of PULL requests sent for downstream traffic */
static uint32_t meas_dw_ack_rcv =
    0; /* number of PULL requests acknowledged for downstream traffic */
static uint32_t meas_dw_dgram_rcv =
    0; /* count PULL response packets received for downstream traffic */
static uint32_t meas_dw_network_byte =
    0; /* sum of UDP bytes sent for upstream traffic */
static uint32_t meas_dw_payload_byte =
    0; /* sum of radio payload bytes sent for upstream traffic */
static uint32_t meas_nb_tx_ok = 0; /* count packets emitted successfully */
static uint32_t meas_nb_tx_fail =
    0; /* count packets were TX failed for other reasons */
static uint32_t meas_nb_tx_requested =
    0; /* count TX request from server (downlinks) */
static uint32_t meas_nb_tx_rejected_collision_packet =
    0; /* count packets were TX request were rejected due to collision with
          another packet already programmed */
static uint32_t meas_nb_tx_rejected_collision_beacon =
    0; /* count packets were TX request were rejected due to collision with a
          beacon already programmed */
static uint32_t meas_nb_tx_rejected_too_late =
    0; /* count packets were TX request were rejected because it is too late to
          program it */
static uint32_t meas_nb_tx_rejected_too_early =
    0; /* count packets were TX request were rejected because timestamp is too
          much in advance */
static uint32_t meas_nb_beacon_queued =
    0; /* count beacon inserted in jit queue */
static uint32_t meas_nb_beacon_sent =
    0; /* count beacon actually sent to concentrator */
static uint32_t meas_nb_beacon_rejected =
    0; /* count beacon rejected for queuing */

static pthread_mutex_t mx_meas_gps =
    PTHREAD_MUTEX_INITIALIZER;        /* control access to the GPS statistics */
...
static pthread_mutex_t mx_stat_rep =
    PTHREAD_MUTEX_INITIALIZER; /* control access to the status report */
static bool report_ready =
    false; /* true when there is a new report to send to the server */
static char status_report[STATUS_SIZE]; /* status report as a JSON object */

/* beacon parameters */
static uint32_t beacon_period =
    0; /* set beaconing period, must be a sub-multiple of 86400, the nb of sec
          in a day */
static uint32_t beacon_freq_hz =
    DEFAULT_BEACON_FREQ_HZ; /* set beacon TX frequency, in Hz */
static uint8_t beacon_freq_nb =
    DEFAULT_BEACON_FREQ_NB; /* set number of beaconing channels beacon */
static uint32_t beacon_freq_step =
    DEFAULT_BEACON_FREQ_STEP; /* set frequency step between beacon channels, in
                                 Hz */
static uint8_t beacon_datarate =
    DEFAULT_BEACON_DATARATE; /* set beacon datarate (SF) */
static uint32_t beacon_bw_hz =
    DEFAULT_BEACON_BW_HZ; /* set beacon bandwidth, in Hz */
static int8_t beacon_power =
    DEFAULT_BEACON_POWER; /* set beacon TX power, in dBm */
static uint8_t beacon_infodesc =
    DEFAULT_BEACON_INFODESC; /* set beacon information descriptor */

/* auto-quit function */
static uint32_t autoquit_threshold =
    0; /* enable auto-quit after a number of non-acknowledged PULL_DATA (0 =
          disabled)*/
...
static uint32_t
    tx_freq_min[LGW_RF_CHAIN_NB]; /* lowest frequency supported by TX chain */
static uint32_t
    tx_freq_max[LGW_RF_CHAIN_NB]; /* highest frequency supported by TX chain */
...
static int parse_SX1301_configuration(const char *conf_file) {
  ...
  if (conf_obj == NULL) {
    ...
    MSG("INFO: %s does contain a JSON object named %s, parsing SX1301 "
        "parameters\n",
  ...
  memset(&boardconf, 0,
         sizeof boardconf); /* initialize configuration structure */
  val = json_object_get_value(conf_obj,
                              "lorawan_public"); /* fetch value (if possible) */
  ...
  val =
      json_object_get_value(conf_obj, "clksrc"); /* fetch value (if possible) */
  ...
  conf_lbt_obj = json_object_get_object(
      conf_obj, "lbt_cfg"); /* fetch value (if possible) */
  if (conf_lbt_obj == NULL) {
    MSG("INFO: no configuration for LBT\n");
  } else {
    val = json_object_get_value(conf_lbt_obj,
                                "enable"); /* fetch value (if possible) */
    if (json_value_get_type(val) == JSONBoolean) {
    ...
    if (lbtconf.enable == true) {
      val = json_object_get_value(
          conf_lbt_obj, "rssi_target"); /* fetch value (if possible) */
      if (json_value_get_type(val) == JSONNumber) {
        lbtconf.rssi_target = (int8_t)json_value_get_number(val);
      } else {
        MSG("WARNING: Data type for lbt_cfg.rssi_target seems wrong, please "
            "check\n");
        lbtconf.rssi_target = 0;
      }
      val = json_object_get_value(
          conf_lbt_obj, "sx127x_rssi_offset"); /* fetch value (if possible) */
      if (json_value_get_type(val) == JSONNumber) {
        lbtconf.rssi_offset = (int8_t)json_value_get_number(val);
      } else {
        MSG("WARNING: Data type for lbt_cfg.sx127x_rssi_offset seems wrong, "
            "please check\n");
      ...
      for (i = 0; i < (int)lbtconf.nb_channel; i++) {
        ...
        /* Channel frequency */
        val = json_object_dotget_value(
            conf_lbtchan_obj, "freq_hz"); /* fetch value (if possible) */
        if (json_value_get_type(val) == JSONNumber) {
          lbtconf.channels[i].freq_hz = (uint32_t)json_value_get_number(val);
        } else {
          MSG("WARNING: Data type for lbt_cfg.channels[%d].freq_hz seems "
              "wrong, please check\n",
              i);
        ...
        /* Channel scan time */
        val = json_object_dotget_value(
            conf_lbtchan_obj, "scan_time_us"); /* fetch value (if possible) */
        if (json_value_get_type(val) == JSONNumber) {
          lbtconf.channels[i].scan_time_us =
              (uint16_t)json_value_get_number(val);
        } else {
          MSG("WARNING: Data type for lbt_cfg.channels[%d].scan_time_us seems "
              "wrong, please check\n",
              i);
  ...
  val = json_object_get_value(conf_obj,
                              "antenna_gain"); /* fetch value (if possible) */
  if (val != NULL) {
    if (json_value_get_type(val) == JSONNumber) {
  ...
  for (i = 0; i < TX_GAIN_LUT_SIZE_MAX; i++) {
    snprintf(param_name, sizeof param_name, "tx_lut_%i",
             i); /* compose parameter path inside JSON structure */
    val = json_object_get_value(conf_obj,
                                param_name); /* fetch value (if possible) */
    if (json_value_get_type(val) != JSONObject) {
      MSG("INFO: no configuration for tx gain lut %i\n", i);
      continue;
    }
    txlut.size++; /* update TX LUT size based on JSON object found in
                     configuration file */
    ...
    if (json_value_get_type(val) == JSONNumber) {
      ...
      txlut.lut[i].dac_gain =
          3; /* This is the only dac_gain supported for now */
    }
  ...
  for (i = 0; i < LGW_RF_CHAIN_NB; ++i) {
    ...
    snprintf(param_name, sizeof param_name, "radio_%i",
             i); /* compose parameter path inside JSON structure */
    val = json_object_get_value(conf_obj,
                                param_name); /* fetch value (if possible) */
    if (json_value_get_type(val) != JSONObject) {
    ...
    if (rfconf.enable == false) { /* radio disabled, nothing else to parse */
      ...
      MSG("INFO: radio %i enabled (type %s), center frequency %u, RSSI offset "
          "%f, tx enabled %d, tx_notch_freq %u\n",
  ...
  for (i = 0; i < LGW_MULTI_NB; ++i) {
    ...
    /* there is an object to configure that Lora multi-SF channel, let's parse
     * it */
    ...
    if (ifconf.enable ==
        false) { /* Lora multi-SF channel disabled, nothing else to parse */
      ...
      MSG("INFO: Lora multi-SF channel %i>  radio %i, IF %i Hz, 125 kHz bw, SF "
          "7 to 12\n",
  ...
  memset(&ifconf, 0, sizeof ifconf); /* initialize configuration structure */
  val = json_object_get_value(conf_obj,
                              "chan_Lora_std"); /* fetch value (if possible) */
  if (json_value_get_type(val) != JSONObject) {
    ...
    if (ifconf.enable == false) {
      ...
      ifconf.rf_chain =
          (uint32_t)json_object_dotget_number(conf_obj, "chan_Lora_std.radio");
      ifconf.freq_hz =
          (int32_t)json_object_dotget_number(conf_obj, "chan_Lora_std.if");
  ...
  memset(&ifconf, 0, sizeof ifconf); /* initialize configuration structure */
  val = json_object_get_value(conf_obj,
                              "chan_FSK"); /* fetch value (if possible) */
  if (json_value_get_type(val) != JSONObject) {
    ...
    if (ifconf.enable == false) {
      ...
      ifconf.rf_chain =
          (uint32_t)json_object_dotget_number(conf_obj, "chan_FSK.radio");
      ifconf.freq_hz =
      ...
      ifconf.datarate =
          (uint32_t)json_object_dotget_number(conf_obj, "chan_FSK.datarate");
...
static int parse_gateway_configuration(const char *conf_file) {
  ...
  if (conf_obj == NULL) {
    ...
    MSG("INFO: %s does contain a JSON object named %s, parsing gateway "
        "parameters\n",
        conf_file, conf_obj_name);
  }
...
static int send_tx_ack(uint8_t token_h, uint8_t token_l,
  ...
  if (error != JIT_ERROR_OK) {
    ...
    switch (error) {
      ...
      case JIT_ERROR_COLLISION_PACKET:
        memcpy((void *)(buff_ack + buff_index), (void *)"\"COLLISION_PACKET\"",
               18);
        buff_index += 18;
        /* update stats */
      ...
      case JIT_ERROR_COLLISION_BEACON:
        memcpy((void *)(buff_ack + buff_index), (void *)"\"COLLISION_BEACON\"",
               18);
        buff_index += 18;
        /* update stats */
        pthread_mutex_lock(&mx_meas_dw);
...
int main(void) {
  ...
  char *global_cfg_path =
      "global_conf.json"; /* contain global (typ. network-wide) configuration */
  char *local_cfg_path =
      "local_conf.json"; /* contain node specific configuration, overwrite
                            global parameters for parameters that are defined in
                            both */
  char *debug_cfg_path =
      "debug_conf.json"; /* if present, all other configuration files are
                            ignored */
...
/* display host endianness */
  ...
  if (access(debug_cfg_path, R_OK) ==
      0) { /* if there is a debug conf, parse only the debug conf */
    ...
    if (x != 0) {
      exit(EXIT_FAILURE);
             ...
             0) { /* if there is a global conf, parse it and then try to parse
                     local conf  */
  ...
  } else if (access(local_cfg_path, R_OK) ==
             0) { /* if there is only a local conf, parse it and that's all */
    MSG("INFO: found local configuration file %s, parsing it\n",
        local_cfg_path);
    ...
    MSG("ERROR: [main] failed to find any configuration file named %s, %s OR "
        "%s\n",
  ...
  if (gps_tty_path[0] !=
      '\0') { /* do not try to open GPS device if no path set */
    i = lgw_gps_enable(gps_tty_path, "ubx7", 0,
                       &gps_tty_fd); /* HAL only supports u-blox 7 for now */
    if (i != LGW_GPS_SUCCESS) {
      printf(
          "WARNING: [main] impossible to open %s for GPS sync (check "
          "permissions)\n",
          gps_tty_path);
      gps_enabled = false;
      gps_ref_valid = false;
  ...
  hints.ai_family = AF_INET; /* WA: Forcing IPv4 as AF_UNSPEC makes connection
                                on localhost to fail */
  ...
  if (q == NULL) {
    MSG("ERROR: [up] failed to open socket to any of server %s addresses (port "
        "%s)\n",
  ...
  if (q == NULL) {
    MSG("ERROR: [down] failed to open socket to any of server %s addresses "
        "(port %s)\n",
        serv_addr, serv_port_up);
    i = 1;
    for (q = result; q != NULL; q = q->ai_next) {
  ...
  while (!exit_sig && !quit_sig) {
    ...
    if (gps_enabled == true) {
      ...
      printf(
          "# GPS *FAKE* coordinates: latitude %.5f, longitude %.5f, altitude "
          "%i m\n",
          cp_gps_coord.lat, cp_gps_coord.lon, cp_gps_coord.alt);
    } else {
    ...
    if (((gps_enabled == true) && (coord_ok == true)) ||
      ...
      snprintf(status_report, STATUS_SIZE,
               "\"stat\":{\"time\":\"%s\",\"lati\":%.5f,\"long\":%.5f,\"alti\":"
               "%i,\"rxnb\":%u,\"rxok\":%u,\"rxfw\":%u,\"ackr\":%.1f,\"dwnb\":%"
               "u,\"txnb\":%u}",
      ...
      snprintf(status_report, STATUS_SIZE,
               "\"stat\":{\"time\":\"%s\",\"rxnb\":%u,\"rxok\":%u,\"rxfw\":%u,"
               "\"ackr\":%.1f,\"dwnb\":%u,\"txnb\":%u}",
...
void thread_up(void) {
  ...
  struct lgw_pkt_rx_s
      rxpkt[NB_PKT_MAX];  /* array containing inbound packets + metadata */
  ...
  struct tref
      local_ref; /* time reference used for UTC <-> timestamp conversion */
  ...
  i = setsockopt(sock_up, SOL_SOCKET, SO_RCVTIMEO, (void *)&push_timeout_half,
  ...
  *(uint32_t *)(buff_up + 8) = net_mac_l;
  ...
  while (!exit_sig && !quit_sig) {
    ...
    send_report =
        report_ready; /* copy the variable so it doesn't change mid-function */
    ...
    for (i = 0; i < nb_pkt; ++i) {
      ...
      switch (p->status) {
        ...
        case STAT_NO_CRC:
          ...
          break;
        default:
          MSG("WARNING: [up] received packet with unknown status %u (size %u, "
              "modulation %u, BW %u, DR %u, RSSI %.1f)\n",
      ...
      if (ref_ok == true) {
        ...
        if (j == LGW_GPS_SUCCESS) {
          pkt_gps_time_ms =
              pkt_gps_time.tv_sec * 1E3 + pkt_gps_time.tv_nsec / 1E6;
          j = snprintf(
              (char *)(buff_up + buff_index), TX_BUFF_SIZE - buff_index,
              ",\"tmms\":%llu",
      ...
      if (p->modulation == MOD_LORA) {
        memcpy((void *)(buff_up + buff_index), (void *)",\"modu\":\"LORA\"",
               14);
        buff_index += 14;
        ...
        switch (p->datarate) {
          case DR_LORA_SF7:
            memcpy((void *)(buff_up + buff_index), (void *)",\"datr\":\"SF7",
                   12);
            buff_index += 12;
            break;
          case DR_LORA_SF8:
            memcpy((void *)(buff_up + buff_index), (void *)",\"datr\":\"SF8",
                   12);
            buff_index += 12;
            break;
          case DR_LORA_SF9:
            memcpy((void *)(buff_up + buff_index), (void *)",\"datr\":\"SF9",
                   12);
            buff_index += 12;
            break;
          case DR_LORA_SF10:
            memcpy((void *)(buff_up + buff_index), (void *)",\"datr\":\"SF10",
                   13);
            buff_index += 13;
            break;
          case DR_LORA_SF11:
            memcpy((void *)(buff_up + buff_index), (void *)",\"datr\":\"SF11",
                   13);
            buff_index += 13;
            break;
          case DR_LORA_SF12:
            memcpy((void *)(buff_up + buff_index), (void *)",\"datr\":\"SF12",
                   13);
            buff_index += 13;
            break;
          default:
            MSG("ERROR: [up] lora packet with unknown datarate\n");
            memcpy((void *)(buff_up + buff_index), (void *)",\"datr\":\"SF?",
                   12);
            buff_index += 12;
            exit(EXIT_FAILURE);
        }
        ...
        switch (p->coderate) {
          case CR_LORA_4_5:
            memcpy((void *)(buff_up + buff_index), (void *)",\"codr\":\"4/5\"",
                   13);
            buff_index += 13;
            break;
          case CR_LORA_4_6:
            memcpy((void *)(buff_up + buff_index), (void *)",\"codr\":\"4/6\"",
                   13);
            buff_index += 13;
            break;
          case CR_LORA_4_7:
            memcpy((void *)(buff_up + buff_index), (void *)",\"codr\":\"4/7\"",
                   13);
            buff_index += 13;
            break;
          case CR_LORA_4_8:
            memcpy((void *)(buff_up + buff_index), (void *)",\"codr\":\"4/8\"",
                   13);
            buff_index += 13;
            break;
          case 0: /* treat the CR0 case (mostly false sync) */
            memcpy((void *)(buff_up + buff_index), (void *)",\"codr\":\"OFF\"",
                   13);
            buff_index += 13;
            break;
          default:
            MSG("ERROR: [up] lora packet with unknown coderate\n");
            memcpy((void *)(buff_up + buff_index), (void *)",\"codr\":\"?\"",
                   11);
            buff_index += 11;
            exit(EXIT_FAILURE);
        }
    ...
    /* restart fetch sequence without sending empty JSON if all packets have
     * been filtered out */
    if (pkt_in_dgram == 0) {
      if (send_report == true) {
...
void thread_down(void) {
  ...
  uint8_t token_l; /* random token for acknowledgement matching */
  bool req_ack =
      false; /* keep track of whether PULL_DATA was acknowledged or not */
  ...
  struct tref
      local_ref; /* time reference used for GPS <-> timestamp conversion */
  ...
  struct timespec
      last_beacon_gps_time; /* gps time of last enqueued beacon packet */
  int retry;
  ...
  /* auto-quit variable */
  uint32_t autoquit_cnt =
      0; /* count the number of PULL_DATA sent since the latest PULL_ACK */
  ...
  *(uint32_t *)(buff_req + 8) = net_mac_l;
  ...
  if (field_latitude > (int32_t)0x007FFFFF) {
    field_latitude =
        (int32_t)0x007FFFFF; /* +90 N is represented as 89.99999 N */
  } else if (field_latitude < (int32_t)0xFF800000) {
    field_latitude = (int32_t)0xFF800000;
  }
  field_longitude =
      (int32_t)((reference_coord.lon / 180.0) * (double)(1 << 23));
  if (field_longitude > (int32_t)0x007FFFFF) {
    field_longitude =
        (int32_t)0x007FFFFF; /* +180 E is represented as 179.99999 E */
  ...
  field_crc2 =
      crc16((beacon_pkt.payload + 6 + beacon_RFU1_size), 7 + beacon_RFU2_size);
  ...
  while (!exit_sig && !quit_sig) {
    ...
    while ((int)difftimespec(recv_time, send_time) < keepalive_time) {
      ...
      while (beacon_loop && (beacon_period != 0)) {
        ...
        if ((gps_ref_valid == true) && (xtal_correct_ok == true)) {
          ...
          if (last_beacon_gps_time.tv_sec == 0) {
             ...
             */
            diff_beacon_time =
                time_reference_gps.gps.tv_sec % ((time_t)beacon_period);
          ...
          /* now we can add a beacon_period to the reference to get next beacon
           * GPS time */
          ...
          /* convert GPS time to concentrator time, and set packet counter for
           * JiT trigger */
          ...
          if (beacon_freq_nb > 1) {
            beacon_chan = (next_beacon_gps_time.tv_sec / beacon_period) %
                          beacon_freq_nb; /* floor rounding */
          } else {
            beacon_chan = 0;
          }
          /* Compute beacon frequency */
          beacon_pkt.freq_hz =
              beacon_freq_hz + (beacon_chan * beacon_freq_step);

          /* load time in beacon payload */
          beacon_pyld_idx = beacon_RFU1_size;
          beacon_pkt.payload[beacon_pyld_idx++] =
              0xFF & next_beacon_gps_time.tv_sec;
          beacon_pkt.payload[beacon_pyld_idx++] =
              0xFF & (next_beacon_gps_time.tv_sec >> 8);
          beacon_pkt.payload[beacon_pyld_idx++] =
              0xFF & (next_beacon_gps_time.tv_sec >> 16);
          beacon_pkt.payload[beacon_pyld_idx++] =
              0xFF & (next_beacon_gps_time.tv_sec >> 24);

          /* calculate CRC */
          field_crc1 =
              crc16(beacon_pkt.payload,
          ...
          if (jit_result == JIT_ERROR_OK) {
            ...
            last_beacon_gps_time.tv_sec =
                next_beacon_gps_time
                    .tv_sec; /* keep this beacon time as reference for next one
                                to be programmed */
            ...
            pthread_mutex_unlock(&mx_meas_dw);
            /* In case previous enqueue failed, we retry one period later until
             * it succeeds */
            /* Note: In case the GPS has been unlocked for a while, there can be
             * lots of retries */
      ...
      /* if no network message was received, got back to listening sock_down
       * socket */
      if (msg_len == -1) {
      ...
      if ((msg_len < 4) || (buff_down[0] != PROTOCOL_VERSION) ||
        ...
        MSG("WARNING: [down] ignoring invalid packet len=%d, "
            "protocol_version=%d, id=%d\n",
      ...
      txpk_obj =
          json_object_get_object(json_value_get_object(root_val), "txpk");
      if (txpk_obj == NULL) {
      ...
      /* Parse "immediate" tag, or target timestamp, or UTC time to be converted
       * by GPS (mandatory) */
      i = json_object_get_boolean(txpk_obj,
                                  "imme"); /* can be 1 if true, 0 if false, or
                                              -1 if not a JSON boolean */
      if (i == 1) {
        ...
        if (val != NULL) {
          ...
          /* Concentrator timestamp is given, we consider it is a Class A
           * downlink */
          ...
          if (val == NULL) {
            MSG("WARNING: [down] no mandatory \"txpk.tmst\" or \"txpk.tmms\" "
                "objects in JSON, TX aborted\n");
            json_value_free(root_val);
            continue;
          }
          if (gps_enabled == true) {
            ...
            if (gps_ref_valid == true) {
              ...
              pthread_mutex_unlock(&mx_timeref);
              MSG("WARNING: [down] no valid GPS time reference yet, impossible "
                  "to send packet on specific GPS time, TX aborted\n");
              json_value_free(root_val);
            ...
            MSG("WARNING: [down] GPS disabled, impossible to send packet on "
                "specific GPS time, TX aborted\n");
            json_value_free(root_val);
      ...
      if (val == NULL) {
        MSG("WARNING: [down] no mandatory \"txpk.freq\" object in JSON, TX "
            "aborted\n");
        json_value_free(root_val);
        continue;
      }
      ...
      if (val == NULL) {
        MSG("WARNING: [down] no mandatory \"txpk.rfch\" object in JSON, TX "
            "aborted\n");
        json_value_free(root_val);
        continue;
      }
      ...
      if (str == NULL) {
        MSG("WARNING: [down] no mandatory \"txpk.modu\" object in JSON, TX "
            "aborted\n");
        json_value_free(root_val);
        continue;
      }
      if (strcmp(str, "LORA") == 0) {
        ...
        if (str == NULL) {
          MSG("WARNING: [down] no mandatory \"txpk.datr\" object in JSON, TX "
              "aborted\n");
          json_value_free(root_val);
          continue;
        }
        ...
        switch (x0) {
          ...
          case 12:
            ...
            break;
          default:
            MSG("WARNING: [down] format error in \"txpk.datr\", invalid SF, TX "
                "aborted\n");
            json_value_free(root_val);
            continue;
        }
        switch (x1) {
          ...
          case 500:
            ...
            break;
          default:
            MSG("WARNING: [down] format error in \"txpk.datr\", invalid BW, TX "
                "aborted\n");
            json_value_free(root_val);
            continue;
        }
        ...
        if (str == NULL) {
          MSG("WARNING: [down] no mandatory \"txpk.codr\" object in json, TX "
              "aborted\n");
          json_value_free(root_val);
          continue;
        }
        ...
        /* parse Lora preamble length (optional field, optimum min value
         * enforced) */
        ...
        if (val == NULL) {
          MSG("WARNING: [down] no mandatory \"txpk.fdev\" object in JSON, TX "
              "aborted\n");
          json_value_free(root_val);
          continue;
        }
        txpkt.f_dev =
            (uint8_t)(json_value_get_number(val) /
                      1000.0); /* JSON value in Hz, txpkt.f_dev in kHz */

        /* parse FSK preamble length (optional field, optimum min value
         * enforced) */
        val = json_object_get_value(txpk_obj, "prea");
        if (val != NULL) {
          i = (int)json_value_get_number(val);
        ...
        MSG("WARNING: [down] invalid modulation in \"txpk.modu\", TX "
            "aborted\n");
        json_value_free(root_val);
        continue;
      }
      ...
      if (val == NULL) {
        MSG("WARNING: [down] no mandatory \"txpk.size\" object in JSON, TX "
            "aborted\n");
        json_value_free(root_val);
        continue;
      }
      ...
      if (str == NULL) {
        MSG("WARNING: [down] no mandatory \"txpk.data\" object in JSON, TX "
            "aborted\n");
        json_value_free(root_val);
        continue;
      }
      ...
      if (i != txpkt.size) {
        MSG("WARNING: [down] mismatch between .size and .data size once "
            "converter to binary\n");
      }
      ...
      if ((txpkt.freq_hz < tx_freq_min[txpkt.rf_chain]) ||
        ...
        MSG("ERROR: Packet REJECTED, unsupported frequency - %u "
            "(min:%u,max:%u)\n",
      ...
      if (jit_result == JIT_ERROR_OK) {
        for (i = 0; i < txlut.size; i++) {
          if (txlut.lut[i].rf_power == txpkt.rf_power) {
            ...
            if (0 != i) {
              ...
              MSG("WARNING: network-server wants to use powe = %d, and "
                  "actually uses powd = %d\n",
        ...
        if (i == txlut.size) {
          ...
          MSG("WARNING: power for TX - %d exceeding maximum power - %d, use "
              "maximum power - %d\n",
              txpkt.rf_power, txlut.lut[txlut.size - 1].rf_power,
              txlut.lut[txlut.size - 1].rf_power);
          txpkt.rf_power = txlut.lut[txlut.size - 1].rf_power;
    ...
    if (jit_result == JIT_ERROR_OK) {
      if (pkt_index > -1) {
        ...
        if (jit_result == JIT_ERROR_OK) {
          ...
          pthread_mutex_lock(
              &mx_concent); /* may have to wait for a fetch to finish */
          ...
          if (result == LGW_HAL_ERROR) {
            ...
            if (tx_status == TX_EMITTING) {
              ...
              MSG("WARNING: a downlink was already scheduled, overwritting "
                  "it...\n");
              print_tx_status(tx_status);
            } else {
              /* Nothing to do */
...
static void modify_os_time(const uint32_t ppm_tstamp) {
  ...
  if (time_diff < 10) {
    ...
    MSG("INFO: [modify_os_time] The difference between the system time(%ld) "
        "and the GPS time(%ld) is less than 10 seconds. Use the system time.\n",
  ...
  if (0 == ret) {
    ...
    MSG("INFO: [modify_os_time] System time has been synchronized via GPS, "
        "%s\n",
        buf);
  }
}
...
void thread_gps(void) {
  ...
  while (!exit_sig && !quit_sig) {
    ...
    ssize_t nb_char =
        read(gps_tty_fd, serial_buff + wr_idx, LGW_GPS_MIN_MSG_SIZE);
    if (nb_char <= 0) {
      ...
      if (serial_buff[rd_idx] == (char)LGW_GPS_UBX_SYNC_CHAR) {
        ...
        if (frame_size > 0) {
          if (latest_msg == INCOMPLETE) {
            ...
            MSG("WARNING: [gps] could not get a valid message from GPS (no "
                "time)\n");
            frame_size = 0;
        ...
        char *nmea_end_ptr =
            memchr(&serial_buff[rd_idx], (int)0x0a, (wr_idx - rd_idx));
        ...
        if (nmea_end_ptr) {
          ...
          if (latest_msg == INVALID || latest_msg == UNKNOWN) {
            ...
            frame_size = 0;
                     ...
                     NMEA_RMC) { /* Get location from RMC frames */
            gps_process_coords();
          }
        }
      }
...
void thread_valid(void) {
  ...
  /* initialization */
  // time(&now_time);
  // strftime(log_name,sizeof
  // log_name,"xtal_err_%Y%m%dT%H%M%SZ.csv",localtime(&now_time)); log_file =
  // fopen(log_name, "w"); setbuf(log_file, NULL);
  // fprintf(log_file,"\"xtal_correct\",\"XERR_INIT_AVG %u XERR_FILT_COEF
  ...
  while (!exit_sig && !quit_sig) {
    ...
    // ref_valid_local?"valid":"invalid", xtal_correct_ok?"valid":"invalid",
    // xtal_correct); // DEBUG
  }
